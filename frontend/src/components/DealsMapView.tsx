    ...
    dealsByDispensary.forEach((dispensaryDeals, dispensaryId) => {
      ...
      popupContent.innerHTML = `
        ...
        <div class="border-t pt-2 mt-2">
          <div class="text-xs font-semibold text-gray-700 mb-1">Deals (${dispensaryDeals.length}):</div>
          ...
          `).join('')}
          ${dispensaryDeals.length > 3 ? `<div class="text-xs text-gray-500 mt-1">+ ${dispensaryDeals.length - 3} more deals</div>` : ''}
        </div>
      `;
    ...
    <div className="w-full h-[calc(100vh-300px)] min-h-[500px] rounded-lg overflow-hidden shadow-lg border border-gray-200 relative">
      ...
      {deals.length === 0 && (
        <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
          <p className="text-gray-500 text-lg">No deals found to display on map</p>
        </div>
      )}
    </div>
  );
